...
if (!Array.prototype.propabilities) {
    Array.prototype.propabilities = function (len: number) {
        if (len === 1) {
            return this.map(op => [op]);
        } else if (len === 2) {
            return this.map(op1 => this.map(op2 => [op1, op2])).reduce((acc, cur) => [...acc, ...cur], []);
        } else {
            return this.map(op => this.propabilities(len - 1).map(a => [op, ...a])).reduce((acc, cur) => [...acc, ...cur], []);
        }
    };
}
