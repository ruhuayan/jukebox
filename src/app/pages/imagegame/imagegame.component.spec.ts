...
describe('ImagegameComponent', () => {
  ...
  it(`should have 25 thumbs and shuffle at init`, () => {
    ...
    for (let i = 0; i < thumbs.length; i++) {
      if (
        thumbs[i].getAttribute('data-ori') !==
        thumbs[i].getAttribute('data-num')
      ) {
        shuffled = true;
        break;
      }
  ...
  it('should compose an image', async () => {
    ...
    const th = Math.floor(img.height / row),
      tw = Math.floor(img.width / row);
    component.showImage();
    ...
    thumbs.forEach((thumb) => {
      ...
      const marginLeftExpected =
        num < row ** 2 ? (tw + 1) * (num % row) : (tw + 1) * row;
      const marginTopExpected =
        num < row ** 2
          ? (th + 1) * Math.floor(num / row)
          : (th + 1) * (row - 1);
